...
async def _get_run_async(storage: StorageBackend, run_id: str) -> WorkflowRun | None:
    """
    Get a workflow run from storage asynchronously.

    Helper for sync contexts that need to fetch a run.
    """
    if hasattr(storage, "connect"):
        await storage.connect()
...
def _register_dynamic_workflow(workflow_name: str, workflow_code: str) -> WorkflowMetadata | None:
    """
    Execute workflow code to register a dynamic workflow.

    This function executes Python source code that defines a workflow,
    allowing dynamically generated workflows to be registered on Celery workers.

    The code is executed in a namespace that includes:
    - pyworkflow decorators: @workflow, @step
    - pyworkflow primitives: sleep, hook, define_hook, shield, etc.
    - pyworkflow context: get_context, has_context
    - pyworkflow exceptions: FatalError, RetryableError, etc.
    - Common stdlib: datetime, timedelta, json, asyncio
    - User-configured imports via dynamic_workflow_imports config

    Security Note: The application is responsible for validating workflow code
    before passing it to pyworkflow. This function executes arbitrary Python code.

    Args:
        workflow_name: Name of the workflow to register
        workflow_code: Python source code defining the workflow

    Returns:
        WorkflowMetadata if registration succeeded, None otherwise
    """
    import asyncio
    import json
    ...
    logger.info(
        f"Registering dynamic workflow: {workflow_name}",
        workflow_name=workflow_name,
        code_length=len(workflow_code),
    )

    # Build execution namespace with pyworkflow primitives
    ...
    # Add user-configured imports
    user_imports = get_dynamic_workflow_imports()
    namespace.update(user_imports)
    ...
    try:
        ...
        if workflow_meta:
            logger.info(
                f"Successfully registered dynamic workflow: {workflow_name}",
                workflow_name=workflow_name,
            )
            return workflow_meta
        else:
            logger.error(
                f"Workflow code executed but workflow '{workflow_name}' not found in registry",
                workflow_name=workflow_name,
            )
            return None
        ...
        logger.error(
            f"Failed to register dynamic workflow: {workflow_name}",
            workflow_name=workflow_name,
            error=str(e),
            exc_info=True,
        )
...
def start_workflow_task(
    ...
    Args:
        ...
        workflow_code: Optional Python source code for dynamic workflows.
            When provided, this code is executed to register the workflow
    ...
    # If workflow not found and we have code, try to register it dynamically
    if not workflow_meta and workflow_code:
        workflow_meta = _register_dynamic_workflow(workflow_name, workflow_code)
...
def start_child_workflow_task(
    ...
    # If workflow not found, check if child run has workflow_code
    if not workflow_meta:
        storage = _get_storage_backend(storage_config)
        child_run = asyncio.run(_get_run_async(storage, child_run_id))
        if child_run and child_run.workflow_code:
            workflow_meta = _register_dynamic_workflow(workflow_name, child_run.workflow_code)
...
async def _resume_workflow_on_worker(
    ...
    # If workflow not found and run has workflow_code, try to register it dynamically
    if not workflow_meta and run.workflow_code:
        workflow_meta = _register_dynamic_workflow(run.workflow_name, run.workflow_code)
